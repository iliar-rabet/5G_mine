    ...
    def plot_2d(self, Z, Dep, CoWs):
        ...
        for point in Dep:
            plt.plot(point.X, point.Y, 'o',
                 label="BS", color=colors[0])
        for point in CoWs:
            plt.plot(point.X, point.Y, 's',
                 label="CoW",color=colors[1])

        ax.set_xlabel('X-axis')
        _ = ax.set_ylabel('Y-axis')
        # plt.show()
        plt.savefig("mine-dep.pdf", format="pdf", bbox_inches="tight")